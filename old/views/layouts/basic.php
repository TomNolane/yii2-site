...
<head>
	...
	<?php// $this->registerCssFile(yii\helpers\Url::to('http://fonts.googleapis.com/css?family=Open+Sans:300,400,700'), ['media' => 'screen']) ?>
	...
	<?php $this->registerCssFile(Url::to('http://fonts.googleapis.com/css?family=Droid+Sans'), ['position'=> \yii\web\View::POS_HEAD,'rel' => 'stylesheet', 'type'=> 'text/css']);
		$this->registerCssFile(Url::to('http://fonts.googleapis.com/css?family=Lato:400,700,400italic,700italic'), ['position'=> \yii\web\View::POS_HEAD,'rel' => 'stylesheet', 'type'=> 'text/css']);
	?>	
...
<?php $this->beginBody() ?> 
<script>
	preloaderCSS3("нко.рф загружается... пожалуйста подождите...");
</script>
<!-- Fixed navbar -->
				...
				<ul class="nav navbar-nav pull-right">  
					...
					<?php  
						...
						if(strpos($current_url , 'site%2Fprojects') || strpos($current_url , 'site%2Fprojects2') || strpos($current_url , 'site%2Fprojects3') || strpos($current_url , 'site%2Fprojects4')) {
							echo '<li class="dropdown">'.Html::a('Список проектов | услуг<b class="caret"></b>', ['#'], ['class' => 'dropdown-toggle active_link', 'data-toggle' => 'dropdown']).'<ul class="dropdown-menu">'; 
						}
						else {
							echo '<li class="dropdown">'.Html::a('Список проектов | услуг<b class="caret"></b>', ['#'], ['class' => 'dropdown-toggle', 'data-toggle' => 'dropdown']).'<ul class="dropdown-menu">'; 
						}
						...
						if(strpos($current_url , 'site%2Fprojects2')) { 
							echo Html::tag('li',Html::a('Направление: Здоровый образ жизни | ЗОЖ', ['site/projects2'], ''),['class' => 'active']);}
							else { echo Html::tag('li', Html::a('Направление: Здоровый образ жизни | ЗОЖ', ['site/projects2'], ''));  }

						if(strpos($current_url , 'site%2Fprojects3')) { 
						echo Html::tag('li',Html::a('Направление: Гранты, субсидии, гос.заказы', ['site/projects3'], ''),['class' => 'active']);}
						else { echo Html::tag('li', Html::a('Направление: Гранты, субсидии, гос.заказы', ['site/projects3'], ''));  }

						if(strpos($current_url , 'site%2Fprojects4')) { 
						echo Html::tag('li',Html::a('Направление: Информационная поддержка', ['site/projects4'], ''),['class' => 'active']);}
						else { echo Html::tag('li', Html::a('Направление: Информационная поддержка', ['site/projects4'], ''));  }
			...
			<div class="container">
				<div class="row">
					...
					<div class="col-md-6 widget">
						...
						<div class="widget-body">
							<p>"Национальное Общество Качества", НП зарегистрирована по адресу:<br>Москва г, проезд.Огородный, д.5, стр.7, индекс 127254.<br>Председатель Совета директоров организации НЕКОММЕРЧЕСКОЕ ПАРТНЕРСТВО "НАЦИОНАЛЬНОЕ ОБЩЕСТВО КАЧЕСТВА" Мельников Геннадий Владимирович.<br>Основным видом деятельности компании является Консультирование по вопросам качества управления и социального парнёрства всех форм собственности.<br>Также "Национальное Общество Качества", НП работает еще по 18 направлениям. Размер уставного капитала 0 руб.<br> 

Компания "Национальное Общество Качества". Основным заказчиком является Дтсзн Города Москвы.<br> 
...
<!-- JavaScript libs are placed at the end of the document so the pages load faster -->  
	<?php// $this->registerJsFile(yii\helpers\Url::to('http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js'), ['position'=> \yii\web\View::POS_END]) ?>
	
	<?php// $this->registerJsFile(yii\helpers\Url::to('http://netdna.bootstrapcdn.com/bootstrap/3.0.0/js/bootstrap.min.js'), ['position'=> \yii\web\View::POS_END]) ?>
